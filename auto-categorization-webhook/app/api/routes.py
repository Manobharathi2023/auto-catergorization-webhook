...
@router.post(
    ...
    description="Receives a support ticket and returns AI-powered classification."
)
async def classify_ticket(
    ...
    logger.info(
        f"Received classification request for ticket: {ticket.ticket_id}"
    )
    ...
    try:
        ...
        ticket_service.save_classification(
            db,
            result,
            ticket.title,
            ticket.description
        ...
        logger.error(
            f"Classification error for {ticket.ticket_id}: {e}"
        )
...
@router.get(
    ...
    description="Check API, Ollama and database health."
)
...
@router.get(
    ...
    description="Retrieve classification result for a specific ticket."
)
def get_ticket(
    ticket_id: str,
    db: Session = Depends(get_db)
    ...
    log = ticket_service.get_log_by_ticket_id(
        db,
        ticket_id
    )
...
@router.get(
    ...
    summary="Get supported categories and priorities",
    description="Returns all supported categories, subcategories and priorities."
)
def get_supported_categories():
    """
    Returns categories, subcategories and priorities
    for frontend dropdowns and help panels.
    """
